...
1. **Does the commit change build, packaging, dependency resolution, release tooling, compiler configuration, bundler configuration, or artifact generation?**

* **Yes:** use `build`.
* Use `build` ONLY for mechanics required to build, package, or release the project.
* Do not use `chore` when `build` applies.

1. **Does the commit change CI/CD workflows, hosted automation, pipeline definitions, or status-check orchestration?**

* **Yes:** use `ci`.
* Use `ci` ONLY for automation executed by CI/CD systems.

1. **If none of the rules above match, use `chore`.**
...
1. **Match repository history second.**
...
1. **Default to scope in a clear monorepo.**
...
1. **Default to no scope when evidence is weak.**
